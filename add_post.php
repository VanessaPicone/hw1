<?php
    ...
    //verifico l'esistenza dei dati del nuovo Post
    if (!empty($_POST["photo"]) && !empty($_POST["name"]) && !empty($_POST["rating"]) && !empty($_POST["address"]) && 
        !empty($_POST["description"])){
            $error = array();

        #PHOTO
        //controllo che sia un URL valido
        if (filter_var($_POST["photo"], FILTER_VALIDATE_URL)) {
          $photo = mysqli_real_escape_string($conn, $_POST['photo']);
        } else {
          $error[] = "Inserisci un URL valido per la foto.";
        }

        # RATING
        // Controlla che che il rating rispetti il pattern specificato
        if(!preg_match('/^[0-5]$/', $_POST['rating'])) {
            $error[] = "Rating non valido, deve essere un numero da 0 a 5";
        }

        # NAME
        $name = mysqli_real_escape_string($conn, $_POST['name']);
        // Cerco se lil nome esiste già
        $query = "SELECT name FROM postes WHERE name = '$name'";
        ...
        if (count($error) == 0) {
            ...
            $query = "INSERT INTO postes(user_id, photo, name, rating, address, description) VALUES('$userid','$photo', '$name', '$rating', '$address', '$description')";
            
            if (mysqli_query($conn, $query)) {
                mysqli_close($conn);
                header("Location: succ_add.php");
                exit;
            } else {
...
<h1><strong>Aggiungi un nuovo luogo</strong><br/></h1>
</header>

<section id="post">
    <h3><strong>Crea il tuo post</strong></h3>
    <div id="sottotitolo">Condividi con noi i tuoi viaggi e le tue esperienze.</div>

    <form name="post" method="post">
        <div class="add_photo">
            <p>Inserisci l'URL della tua foto</p>
            <input type="url" name="photo" placeholder="https://esempio.com/foto.jpg" required>
            <div><img src="./assets/close.svg"/><span>Inserisci un URL valido</span></div>
        </div>  
        <div class="add_name">
            <p>Aggiungi un Nome</p>
            <input type="text" name="name">
            <div><img src="./assets/close.svg"/><span>Devi inserire un nome</span></div>
        </div> 
        <div class="add_rating">
            <p>Dai una tua valutazione</p>
            <input type="text" name="rating" placeholder="Da 0 a 5...">
            <div><img src="./assets/close.svg"/><span>Devi inserire una valutazione da 0 a 5</span></div>
        </div> 
        <div class="add_address">
            <p>Inserisci un indirizzo valido</p>
            <input type="text" name="address">
            <div><img src="./assets/close.svg"/><span>Devi inserire un indirizzo valido</span></div>
        </div> 
        <div class="add_description">
            <p>Racconta la tua esperienza</p>
            <div>
